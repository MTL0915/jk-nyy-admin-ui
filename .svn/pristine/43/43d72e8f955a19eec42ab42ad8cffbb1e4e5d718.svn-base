<template>
  <div>
    ...
    >
      <a
        style="color:#5b5bca"
        @click="handelWatch(deviceObj.id, null);$message.success('已刷新')"
      >刷新</a>
      <a
        style="color:#5b5bca"
    ...
    <div
      :style="{ height: checkHide()?'100%':'calc(100% - 53px)', top: checkHide()?'0px':'45px' }"
      class="app_info open"
    >
      <div
        class="cover"
      ...
      <div class="app_box">
          ...
          >
            ...
            >
              ...
              >
                ...
                <div>
                  <span style="color:#605555;">{{ deviceObj.device_id }}</span>
                  <span>(版本号：{{deviceObj.version}})</span>
                </div>
              </div>
              <!-- 网关设备 -->
              <div
                class="model_content_row"
                style="padding:0px;"
                v-if="childDeviceList.length !== 0"
              >
                <i
                  class="el-icon-share row_centent_img"
                  ...
                  <el-table :data="childDeviceList">
                    ...
                    >
                      <template slot-scope="scope">
                        {{formatDate(new Date(scope.row.system_response_time), 'yyyy-MM-dd hh:mm:ss')}}
                      </template>
                    </el-table-column>
                    <el-table-column
                      label="操作"
                    ...
                    >
                      <template slot-scope="scope">
                        <el-button
                          @click="viewChildDevice(scope.row)"
                          type="text"
                          size="small"
                        >查看</el-button>
                      </template>
              ...
              <div
                class="model_content_row"
                v-if="deviceObj.hd_device_parent_name"
              >
                <i
                  class="el-icon-upload row_centent_img"
                  style="font-size:15px;color:#333;"
                ></i>
                <span class="row_centent">{{ deviceObj.hd_device_parent_name }}</span>
              </div>
              ...
              <div
                class="model_content_row"
                style="cursor: pointer"
                v-show="enterIntoGwShow"
              >
                <i
                  class="el-icon-eleme row_centent_img"
                  style="font-size:15px;color:#333;"
                />
                <span
                  class="row_centent"
              ...
              <el-button
                ...
                icon="el-icon-edit"
                size="mini"
                style="background: #1EC5E1;border-color: #1EC5E1;display: none;"
              >编辑</el-button>
            </div>
          </div>
          <!-- 设备日志 -->
        ...
        <div class="app_box_right">
          ...
          <div
            class="model"
            v-if="deviceObj.hd_device_type_code === 'JK-LD'"
            style="margin-bottom:0;width: 100%;position: relative;"
          >
            <div class="model_name">LED预览</div>
            <div style="margin-top:15px;background: #000000f7;">
              <div style="width:60%;border-radius: 30px;">
                <LedConfig
                  ref="led"
                  style="border-right: 2px solid #151414;"
                />
              </div>
            </div>
          </div>
          ...
          <!-- 新水肥设备 -->
          <div
            v-if="deviceObj && deviceObj.hd_device_type_code === 'JK-FM' && jkfmOnLine"
            class="model"
            style="width: 100%;position: relative;"
          >
            <div class="model_name">水肥机配置</div>
            <div style="margin-top:5px;">
              <JkfmDevice
                :device_id="deviceObj.device_id"
                :offLineFunction="jkfmOffLineFunction"
              />
            </div>
          </div>
          ...
          <div
            ...
            v-if="recognitionDataShow"
          >
            <div class="model_name">识别统计</div>
            <recognitionStatistics
              :device_ids="deviceObj.device_id"
              :recognitionDataStartTime="recognitionDataStartTime"
              :recognitionDataEndTime="recognitionDataEndTime"
            />
          </div>
          ...
          <div
            class="model"
            style="width: 100%;height: 400px;position: relative;"
            v-if="batteryLeftbatteryLeftShow"
          >
            <div class="model_name">电量统计</div>
            <batteryLeftbatteryLeft
              :device_id="deviceObj.device_id"
              :channels="batteryLeftbatteryLeftChannels"
            />
          </div>-->
          ...
          <div
            ...
            v-if="simFlowShow"
          >
            <div class="model_name">流量统计</div>
            <simFlow :device_id="deviceObj.device_id" />
          </div>

          <!-- 农机数据列表 -->
          <div
            class="model"
            v-if="deviceObj.device_type === 'JK-NJ' || deviceObj.device_type === 'JT-NJ' "
            style="width: 100%;position: relative;"
          >
            <div class="app_box_bottom_left">
              <div class="model_name">农机数据列表</div>
              <div>
                <njDataList
                  ref="njDataList"
                  :device_id="deviceObj.device_id"
    ...
    <form
      ...
      method="post"
      v-if="enterIntoGwType===2"
      target="_blank"
    >
      <input
        type="text"
        name="username"
        :value="enterIntoGwUsername"
      >
      <input
        type="password"
        name="password"
        :value="enterIntoGwPassword"
      >
      <input
        type="hidden"
        name="redirectURL"
    ...
    <form
      ref="ruifengForm"
      action="http://agri120.org/user/login.html"
      method="post"
      v-if="enterIntoGwType===1"
      target="_blank"
    >
      <input
        type="text"
...
import { getDetailById, listByHd_device_parent_id, valveOpenTimeSort, getDeviceRunLogListByh_id } from '@/api/equip';
...
import njDataList from '../component/common/njDataList';
import { formatDate, parseDate } from '@/utils/date'
import recognitionStatistics from './recognitionStatistics'
import { getNewestData as getRecognitionNewestData } from "@/api/recognition";
import JkfmDevice from '@/views/base/equip/module/jkfm/JkfmDevice'
import simFlow from '@/views/base/equip/simFlow'
...
export default {
  ...
  data () {
    return {
      ...
      sensorKz: [],
      sensorCj: [],
      enterIntoGwShow: false,//进入官网
      enterIntoGwType: 0,//1瑞丰官网 2托普云农官网
      enterIntoGwUsername: null,
      enterIntoGwPassword: null,
      recognitionDataShow: false,//存在识别数据
      recognitionDataStartTime: null,//识别数据开始时间
      recognitionDataEndTime: null,//识别数据结束时间
      jkfmOnLine: true,//新水肥机在线状态(不在线时将组件框进行隐藏)
      // batteryLeftbatteryLeftShow: false,//电表耗电量展示
      // batteryLeftbatteryLeftChannels: [],//电表耗电量对应的通道
      simFlowShow: false,//显示流量数据
    };
  },
  filters: {
    ellipsis (value) {
  ...
  mounted () {
    ...
    if (hd_device_id != null && hd_device_id != '') {
      this.handelWatch(hd_device_id, null);
    }
    var device_id = this.$route.query.device_id;
    if (device_id != null && device_id != '') {
      this.handelWatch(null, device_id);
    }
  },
  methods: {
    formatDate,
    ...
    //新水肥机离线触发此方法
    jkfmOffLineFunction () {
      this.jkfmOnLine = false
    },
    //查看子设备
    viewChildDevice (row) {
      this.$router.push({ path: '/deviceInfo?device_id=' + row.device_id })
    },
    //免登录进来隐藏其中一些东西
    checkHide () {
      if (this.$route.path === "/deviceDetails") {
        return true;
      }
      return false;
    },
    ...
    checkModuleShow () {
      this.$nextTick(() => {
        ...
        this.deviceInteriorShow = false//内部状态
        this.enterIntoGwShow = false //第三方设备官网
        let code = this.deviceObj.hd_device_type_code

        // //根据传感器判断展示或隐藏的模块
        // this.batteryLeftbatteryLeftShow = false
        // this.batteryLeftbatteryLeftChannels = []
        // if (this.deviceObj.sensor) {
        //   for (let i = 0; i < this.deviceObj.sensor.length; i++) {
        //     let s = this.deviceObj.sensor[i]
        //     //正向有功功率
        //     if (s.hd_sensor_type_function_code === '123_OTHER1') {
        //       this.batteryLeftbatteryLeftChannels.push(s.channel)
        //     }
        //   }
        //   if (this.batteryLeftbatteryLeftChannels.length !== 0) {
        //     this.batteryLeftbatteryLeftShow = true
        //   }
        // }

        //判断是否绑定sim卡
        if (this.deviceObj.hd_device_sim_id) {
          this.simFlowShow = true
        }

        //判断是否展示内部模块
        if (this.deviceObj.hd_device_type_subordinate !== 'other') {
          this.deviceInteriorShow = true
        }
        ...
        if (code === 'JK-LD') {//LED屏
          ...
          if (this.communication) {
            ...
            if (type === 'JK-CQD1') {//瑞丰虫情灯 
              // this.enterIntoGwShow = true
              // this.enterIntoGwType = 1
              // this.enterIntoGwUsername = comm_json.config.user_name
              // this.enterIntoGwPassword = comm_json.config.password
              this.deviceLogShow = false
            } else if (type === 'JK-CQD2') {//托普云农虫情灯
              this.enterIntoGwShow = true
              this.enterIntoGwType = 2
              this.enterIntoGwUsername = comm_json.config.user_name
              this.enterIntoGwPassword = comm_json.config.password
              this.deviceLogShow = false
            }
        ...
        //是否显示识别模块
        getRecognitionNewestData({
          device_id: this.device_id
        }).then(res => {
          if (res.code === 200) {
            if (res.data !== null) {
              let date = new Date(res.data.time)
              this.recognitionDataEndTime = this.formatDate(date, 'yyyy-MM-dd hh:mm:ss')//默认此刻
              this.recognitionDataStartTime = this.formatDate(new Date(date.getTime() - (7 * 24 * 60 * 60 * 1000)), 'yyyy-MM-dd hh:mm:ss')//默认七天前
              this.recognitionDataShow = true
            } else {
              this.recognitionDataShow = false
            }
          } else {
    ...
    //获取设备详情
    getDetailById () {
      getDetailById({ hd_device_id: this.hd_device_id, device_id: this.device_id }).then(res => {
        this.deviceObj = res.data
        this.hd_device_id = res.data.id
        this.device_id = res.data.device_id
        this.communication = res.data.communication
        if (res.data.subNum !== 0) {
          listByHd_device_parent_id({
            device_id: this.device_id
          }).then(res2 => {
            if (res2.code === 200) {
              this.childDeviceList = res2.data
            } else {
    ...
    handelSwitch (row, scope) {
      // //lzq测试
      // getFormula('FM01A-0000000', this.$ws).then(res => {
      //   
      // })
      // // setFormula('FM01A-0000000', null, this.$ws).then(res => {
      // //   
      // // })
      // return
      // //lzq测试
