...
export default {
  ...
  methods: {
    ...
    showTrigger (row) {
      this.$refs.triggerPanel.showPanel(this.parentObj);
      this.$refs.triggerPanel.strategy = {
        hd_device_sensor_id: "",
        notice_phones: [],
        notice: false,
        phone_notice: false,
        upper: row.start_value,
        lower: row.end_value
      }
    },
    subDelete (number) {
    ...
    closeDialog () {
      ...
      for (let i = 0; i < this.tableData.length; i++) {
        if (!this.tableData[i].start_value || !this.tableData[i].end_value) {
          this.tableData.splice(i, 1)
          i = 0;
        }
      }
      this.$parent.form.sensorList.forEach((v, i) => {
        if (this.id === v.id) {

          v.start_value = this.start_value;
          v.end_value = this.end_value;
