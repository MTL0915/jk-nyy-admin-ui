define([
	...
	return {
		...
		updateGraphicImage : function(e){
			...
			input.onchange = function(e){
				...
				f.onload = function(e){
					...
					dom.src=e.currentTarget.result;
				}.bind(this);
				f.readAsDataURL(e.currentTarget.files[0]);
			}.bind(this);
			input.click();
		},
		onchange_updateGraphicImage : function(e){
			var f = new FileReader();
			f.onload = this.onchange_updateGraphicImage_FileReader.bind(this);
			f.readAsDataURL(e.currentTarget.files[0]);
		},
		onchange_updateGraphicImage_FileReader : function(e){
			this.graphic.symbol.setUrl(e.currentTarget.result);
			this.graphic.getLayer().refresh();
		},
		//修改图标大小
		updateGraphicImageSize : function(e){
			this.graphic.symbol.setHeight(e.target.value);
			this.graphic.symbol.setWidth(e.target.value);
			this.graphic.getLayer().refresh()
		},
		...
		initOption : function(){
			...
			dom.getElementsByClassName('graphicImage')[0].src = gra.symbol.url;
			dom.getElementsByClassName('graphicImageSize')[0].value = gra.symbol.size;
		...
		//切换成为填充symbol
		updateSymbolTypeToFill : function(e){
			var gra = this.graphic;
			if( confirm("如果转化symbol不会保存当前配置") ){
				this.graphic.setSymbol( new esri.symbol.SimpleFillSymbol() );
