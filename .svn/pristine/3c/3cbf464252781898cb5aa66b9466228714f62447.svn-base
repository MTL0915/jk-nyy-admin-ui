<template>
  ...
  >
    ...
    <div class='list'>
      ...
      >
        <div
          v-show='it.data.config.hotType == activeType'
          class='listMode'
        >
          {{ it.data.name }}
          <div class='opt'>
...
export default {
  ...
  methods: {
    ...
    getDataLength () {
      this.dataLength = this.data.filter((e) => { e.data.config.hotType == this.activeType }).length;
    },
    ...
    lookAt (anchor) {
      ...
      var angle = Math.floor(180 / (Math.PI / Math.atan(Math.pow(Math.abs(anchor.position.z), 2) / Math.pow(Math.abs(anchor.position.x), 2))));
      ...
      lon = angle;
      // var xx = Math.sqrt(Math.pow(anchor.position.x, 2) + Math.pow(anchor.position.z, 2));
      // angle = Math.abs(Math.floor(180 / (Math.PI / Math.atan(xx / anchor.position.y))));
