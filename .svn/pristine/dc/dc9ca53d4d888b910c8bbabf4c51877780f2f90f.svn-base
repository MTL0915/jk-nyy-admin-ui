<template>
  <div class="equip_component_div">
    <div class="component_title_div">日均统计</div>
    <div style="display: flex;width:100%;justify-content: space-between">
      ...
      <div>
        ...
        <el-select v-model="type" placeholder="请选择" size="mini" style="width:150px">
          <el-option
...
import { yunmuDateData } from '@/api/yunmu'
...
export default {
  ...
  methods: {
    ...
    loadChart(){
      ...
      yunmuDateData(params).then(res => {
        var data = res.data;
        var xAxis = [];
        var datas = [];
        for (var i = 0;i < data.length;i++){
          xAxis.push(data[i].dateStr);
          datas.push(data[i][this.type]);
        }
        var chartData = {
          xAxis:xAxis,
          datas:datas
