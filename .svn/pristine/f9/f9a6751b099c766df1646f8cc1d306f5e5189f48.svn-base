  ...
  <div class="headBox">
    ...
    <div class="row3">
      <div class="row2_btn">
        ...
        <span v-if="property_values.controlType === 3">
          <img
            :src="
              require('@/assets/img/Plan/drawLineMap/qp/xianchang_control.png')
            "
          />
        </span>
        ...
        <span v-if="property_values.controlType !== 3">
          <img
            :src="
              require('@/assets/img/Plan/drawLineMap/qp/yuancheng_control.png')
            "
          />
        </span>
      </div>
      ...
      >
        ...
        <span v-if="property_values.nav_cmd !== 5">
          <img
            :src="require('@/assets/img/Plan/drawLineMap/qp/hand_control.png')"
          />
        </span>

        <span v-if="property_values.nav_cmd === 5">自动航行</span>
        <span v-if="property_values.nav_cmd === 5" @click="stopRouteJob">
          <img
  ...
  methods: {
    getDevice() {
      ...
      getDevice(device_id).then(res => {
        getDeviceConf(device_id).then(conf => {
          ...
          this.hd_device_type_image_path = this.getImage(
            this.device.hd_device_type_image_path
          );
          // 获取设备上报信息
    ...
    getPower() {
      ...
      if (max === 0) {
        ...
        this.getPowerValue =
          parseInt(((batVol * 1 - min * 1) / (max * 1 - min * 1)) * 100) + "%";
      }
    },
    ...
    deviceWsUpdateFunction(info) {
      ...
      if (this.sensorInfos.length == 0) {
        ...
        this.sensorInfos.forEach(e => {
          ...
          var infos = info.sensorInfos.find(e => {
      ...
      //判断电源是否打开
      var isPower = (this.powerSensor = this.sensorInfos.find(e => {
    ...
    clickPower() {
      ...
      if (value == 1) {
        // 如果这个开关是开着的  则关闭
        ws.clearSensor(it.hd_device_sensor_id || it.id)
          .then(res => {
            if (res.code == 200) {
              this.disPower = false;
              this.isPower = it.value;
        ...
        // 如果这个开关是关着的  则开启
        ws.openSensor(it.hd_device_sensor_id || it.id)
          .then(res => {
            if (res.code == 200) {
              this.disPower = false;
              this.isPower = it.value;
