<template>
  ...
  >
    ...
    >
      <!-- <div
        class='tishituodong'
        :class='{active : showTishituodong1}'
      >
        <div>
          <img draggable="false" src='@/assets/img/vr/drag.png' />
        </div>
        <span style='user-select: none!important;'>点击开启3D视角 拖动享受更多精彩</span>
      </div> -->
    </div>
    <div
      class='cover'
    ...
    <div>
      ...
      >
        ...
        <div class='hotListEdit'>
          <div class='div'>
            <span
              class='hotListEditBtn'
              @click='hotListEditClick(it)'
            >编辑</span>
            <span
              class='hotListDelBtn'
              @click='hotListDelClick(it)'
            >删除</span>
          </div>
          <div class='editWH'>
            <!-- <input v-model="it.config.windowWidth" style='width:50px'/>X<input v-model="it.config.windowHeight" style='width:50px'/> -->
            <!-- <i class='el-icon-close' style='color:#ff0000;' title='删除' /> -->
          </div>
        </div>
        <iframe
...
import { get as getScene, getByPanorId as getSceneByPanorId, edit as editSceneById } from '@/api/rsPanorScene'
...
export default {
  ...
  data: () => ({
    domainName: process.env.VR_IMGPATH,
    consolelog: "测试内容",
    // vr镜面墙
    wall: [],
    // 热点
    ...
    vrCur: false,
    isVrShow: true,
    showTishituodong: false,
    showTishituodong1: false,
  ...
  methods: {
    ...
    init (panorId) {
      ...
      // initMesh();
      this.loadCamera()
      //this.addVrBrowse(panorId)
      // this.mouseVector();
    },
    //增加一次VR浏览量
    // addVrBrowse (panorId) {
    //   addPanorViewCount({ id: panorId }).then(res => {
    //     if (res.code === 200) {
    ...
    jumpScene (param) {
      ...
      var imgLoad = () => {
        index++;
        var jlv = index / 6
        this.$refs.loadingDiv.style.width = parseInt(jlv * 100) + "%";
        if (jlv === 1) {
          setTimeout(() => {
            this.isVrShow = false;
            this.$refs.loadingDiv.style.width = 0 + "%";
            this.openVrCover();
          }, 500)
        }
      }
    ...
    initMesh (param) {
      ...
      this.camera = window.camera = camera = new THREE.PerspectiveCamera(60, _this.$refs.demo1.offsetWidth / _this.$refs.demo1.offsetHeight, 1, 10000)
      this.camera.fov_ = 60
      ...
      //   renderer.setSize(window.innerWidth, window.innerHeight)
      //   _this.consolelog = 'reset';
      // }
      container.appendChild(renderer.domElement)
      var boolStop = false
      // setTimeout(()=>{ boolStop = true; },1000)

      // window.onblur = function(){ boolStop = true; }
      // window.onfocus = function(){ boolStop = true; }
      ...
      var animate = function () {
        ...
        _lat += (__lat - _lat) * 0.3;
        phi = THREE.Math.degToRad(_lat)
        theta = THREE.Math.degToRad(_lon)
      ...
      window.vrBindEvent = () => {
        ...
        function onDocumentMouseWheel (event) {
          ...
          if (camera.fov_ > 100) { camera.fov_ = 100 }
        ...
        function onDocumentTouchMove (event) {
          ...
          if (event.touches.length > 1) {
            ...
            else if (cameraFov > 100) cameraFov = 100;
    ...
    renderAnchor (rsPanorScene, fn) {
      getAnchorBySceneId(rsPanorScene).then((event) => {
        for (var i = 0; i < event.content.length; i++) {
          event.content[i].config = JSON.parse(event.content[i].config)
          this.addAnchor(event.content[i].imagePath, event.content[i])
        }
    ...
    editHot (json) {
      ...
      editAnchor({
        ...
        if (config.hotType === 'list') {
          // 首先查询到修改的热点list
          var hotList = this.iframeHotList.find((e) => { return e.id === config.id });
          // 热点list刷新同步数据
          hotList.config = { ...hotList.config, ...config };
        }
      })
    },
    ...
    getXY (it) {
      ...
      if (!dom) {
        ...
        div.style = 'position: absolute;height: 30px;width:30px;' //transform: translate(-50%, -50%);
        dom = it.dom = div
        var element = document.createElement('div');
        var cover = document.createElement('div');
        cover.style.cssText = "position:absolute;width:100%;height:100%;top:0;left:0;z-index:1;";
        element.appendChild(cover);
        var img = document.createElement("img");
        img.src = it.element.src
      ...
      dom.style.top = (y - 15) + 'px'
    },

    // 屏幕坐标转世界坐标
    ScreenToWorld (x, y) {
      var a = -1;
      const pX = (x / this.renderer.domElement.clientWidth) * 2 - 1;
      const pY = -(y / this.renderer.domElement.clientHeight) * 2 + 1;
    ...
    getXYZ (anchor, event) {
      ...
      const p = this.ScreenToWorld(event.x, event.y);
    ...
    // 热点列表点击进入编辑
    hotListEditClick (param) {
      this.editAnchor({ data: param })
    },

    // 删除热点列表
    hotListDelClick (param) {
      this.deleteAnchor({ data: param })
    },
    ...
    // 显示新窗口内容
    showIframeWindow (anchar, name, url, width, height) {
      var data = anchar.data;
      var config = data.config;
      var name = data.name;
      var url = config.hotLink;
      this.iframeShow = true
      this.iframeSrc = url
      this.iframeTitle = name || '默认弹窗'
      this.iframeWidth = config.windowWindth || 500
      this.iframeHeight = config.windowHeight || 400
    },
    ...
    showDeviceInfo (anchor, device) {
      ...
      dom.innerHTML = `
          <div class='iframeBox' v-show='iframeShow' style='top:-100%;width: 300px;height: 200px;'>
            <div class='iframeTitle'>
              ${anchor.data.name}
              <i class='el-icon-minus closeIframe' style='right: 40px;' onclick='var dom =this.parentElement.parentElement; $(dom).removeClass("full")'></i>
              <i class='el-icon-copy-document closeIframe' style='right: 25px;' onclick='var dom =this.parentElement.parentElement; $(dom).addClass("full")'></i>
            ...
            <div class='iframeWindow' style='width:100%;height:calc(100% - 18px);background: #00000099'>
              <iframe
                src="./deviceshow?hd_device_id=${device}&mode=map3d"
                frameborder="0"
              />
            </div>
          </div>
    ...
    setDefaultPosition () {
      var config_json = { "defaultAngle": { "x": 80, "y": 30 } };
      editSceneById({
        id: this.sceneId,
