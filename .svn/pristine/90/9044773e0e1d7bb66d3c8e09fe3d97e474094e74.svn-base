...
import gyControl from '@/views/devicemap/gyMap/gyControl';
import remoteControl from './remoteControl'
import sensorBtn from './sensor_btn'
export default {
    props: {
        updateInfo: {
            type: Function,
        ...
        // 设备id 因为要第一时间脸上ws 所以需要直接获取url参数第一时间赋值
        device_id: {
            type: String,
            default: ""
        },
    ...
    methods: {
        ...
        clickSensor (it){
            if( it.value == 1 ){
                // 如果这个开关是开着的  则关闭
                this.$refs.control.sendOff(it.hd_device_sensor_id);
            }else{
                // 如果这个开关是关着的  则开启
                this.$refs.control.sendOn(it.hd_device_sensor_id);
            }
        },
        ...
        updateInfo1 (info){
            this.info = info;
            this.$parent.updateInfo = info;
            this.$parent.property_values = info.property_values;
            this.$parent.sensorInfos = info.sensorInfos;
        ...
        openSwitch (){
            // 调整开关状态
            this.switchBtn = this.switchBtn === 0 ? 1 : 0;
        },
        ...
        //     if( this.speed_gear !== undefined && this.speed_gear < 1.5 ){
        //         this.speed_gear += 0.1;
        //         // this.$refs.control.setSpeedGean(++this.speed_gear);
        //     }
        // },

        // // 档位减少
        // downSpeedGean (){
        //     if( this.speed_gear !== undefined && this.speed_gear > 0.1 ){
        //         this.speed_gear -= 0.1;
        //         // this.$refs.control.setSpeedGean(--this.speed_gear);
        //     }
        // },
