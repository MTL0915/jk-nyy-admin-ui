<template>
  <div style="overflow-y:auto;">
    <div class="app_info open">
      ...
      <div class="app_box">
        <div class="app_box_left">
          ...
          >
            <div class="model_name">
              <div class="border_bottoms">{{ facilitiesInfoname }}</div>
            </div>
            <div
              class='goMap'
              style='height:calc(100% - 35px);position: relative;margin-top:10px;'
            >
              <!-- 点击进入一张图 -->
              <div
                class='goMapCover'
          ...
          <div class="model cgq_model">
            ...
            >
              <div class="border_bottoms">设备运行状态</div>
              <!-- 地块日志 -->
                ...
                <el-timeline>
                  ...
                  >
                    <el-card>
                      ...
                      <p style="margin-top: 10px;color:#FFF">{{ item.title }}</p>
                      <div style="margin-top:10px;align-items: center;color:#FFF">
                        <div>
                          <img
                            style="width: 18px;height: 18px;align-self:center;position:absolute"
                            src="/static/img/timer2.png"
                          />
        ...
        <div class="app_box_right">
          ...
          >
            <div class="model_name">
              <div class="border_bottoms">视频监控</div>
            </div>
          ...
          <div
            class="model"
            style="width: calc(50% - 10px);height:35%"
          >
            <div class="model_name">
              <div class="border_bottoms">设备控制</div>
            </div>
            <div
              class="model_content"
              style="height:calc(100% - 35px);"
                ...
                >
                  <table class="table-div">
                    <tbody id="table-tbody">
                      <tr v-for="(item,index) in controlSensor">
                        <td>{{item.hd_device_sensor_name}}</td>
                        <td>{{item.hd_device_sensor_channel}}</td>
                        <td>
                          <!-- 二挡开关 -->
                          <el-switch
                            v-if="item.hd_sensor_type_code == '101'"
                            v-model="item.value"
                            @change="handelSwitch(item, index)"
                            ...
                            :disabled="
                              !checkDeviceControl(item.hd_device_id)
                            "
                          >
                          </el-switch>
                          <!-- 三挡开关 -->
                          ...
                          <el-radio-group
                            v-if="item.hd_sensor_type_code === '102'"
                            v-model="item.value"
                            size="small"
                            v-loading="item.loading"
                            :disabled="
                              !checkDeviceControl(item.hd_device_id)
                            "
                            @change="handelWindow(item, index)"
                          >
                            <el-radio-button
                              key="8"
                              :label="8"
          ...
          <div
            ...
            style="position: relative;height: auto;height: 65%;"
          >
            <div class="model_name">
              <div class="border_bottoms">采集数据</div>
            </div>
            ...
            <div
              v-if="rightTopShow"
              style="height:calc(100% - 35px);align-content:flex-start;flex-wrap: wrap;"
            >
              <div
                class="mini_box"
                style="margin:10px;"
              ...
              >
                <div
                  ...
                  :style="index === rightTopEchartsTypeChoose? 'background: rgba(207, 255, 255, 0.5)': 'background: rgba(240,255,255,0.329)'"
                >
                  <div
                    style="display:flex;position: relative;color:#455a64;height: 100%;cursor:pointer"
                  ...
                  >
                    <div style="flex:1;text-align: center;margin:auto;height: 100%;">
                      ...
                      <div style="height: 35px;">
                        ...
                        >
                          <div style="float:left;color:#0FF">{{ items.hd_device_sensor_name }}<i
              ...
              <sensorEcharts
                ...
                :style="{height: miniBoxHeight? 'calc(100% - 113px)': 'calc(100% - 206px)'}"
                v-if="currentID"
                :ids="currentIds.join(',')"
                :currentID="currentID"
    ...
    <el-dialog
      fullscreen
      append-to-body
      :visible.sync="equipLogShow"
      v-if="equipLogShow"
    >
      <equitLog ref="equipLog" />
    </el-dialog>
  </div>
</template>
...
import equitLog from "@/views/base/equip/module/Log"
...
export default {
  ...
  created () {
    ...
    window.addEventListener("resize", () => {
      this.$nextTick(() => {
        if (this.$refs.miniBox.offsetHeight > 93) {
          this.miniBoxHeight = false;
        } else {
          this.miniBoxHeight = true;
        }
      })
    })
  },
  mounted () {
    var dk_id = this.$route.query.rs_facilities_id;
    this.dk_id = dk_id;
    // 获取地块列表
  ...
  methods: {
      ...
      if (config_jsonStr != null) {
        ...
        for (var i = 0; i < config_json.length; i++) {
          ...
          if (value >= config_json[i].start_value * 1 && value < config_json[i].end_value * 1) {
            console.log(config_json[i].color)
    ...
    showDkMap () {
      ...
      ajaxApi.getFacilitiesByid(this.dk_id, (e) => {
        ...
        // 将地块数据转换为几何
        var graphic = new esri.geometry.Polygon({ rings: dkJson.rings, spatialReference: this.map.spatialReference });
        // 几何墨卡托坐标系转换
        // graphic = esri.geometry.geographicToWebMercator( graphic );
        // 获取几何中心点
        graphic = graphic.getCentroid();
        // 创建图标
        var symbol = new esri.symbol.PictureMarkerSymbol(require("@/assets/img/Plan/biaoshi.png"), 32, 32);
        // 将图标添加到地图中
    ...
    goMap () {
      window.open("../map3d?base_id=" + this.bs_base_id + "&selectDk=" + this.dk_id);
    ...
    loadAllLog () {
      this.equipLogShow = true;
      this.$nextTick(() => {
        this.$refs.equipLog.webType = "router-url"
      })
    },
    loadDKList (dk_id) {
    ...
    show (id) {
      ...
      const _this = this;
      this.getDK(id);
      this.getDeviceRunLogListByr_id(id, this.startPosition, 20);
      ...
      getFacilitiesDetails({
        ...
        this.bs_base_id = res.data.facilitiesInfo.bs_base_id
...
.border_bottoms:after
  ...
  height 2px
  left 0px
  background -webkit-gradient(linear, left top, right top, from(#0ff), to(transparent))
  background linear-gradient(to right, #0ff, transparent)

.el-timeline-item__content .el-card
  background #f0ffff54
  border 1px solid #f0ffff54

.right_top_data
  background rgba(240, 255, 255, .329)
