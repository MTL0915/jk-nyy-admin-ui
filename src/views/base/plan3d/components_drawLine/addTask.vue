<template>
  <div class='addTask'>
    ...
    <div class='addTaskHead'>
      ...
      <span @click='back' style='font-size:14px;'>新增任务</span>
    </div>
    <!-- 身体 -->
    <div class=''>
      <div class='addTaskModel'>
        <div class='addTaskModelName'>设备名称：</div>
        <div>{{publicData.activeDevice.name}}</div>
      </div>
      <div class='addTaskModel'>
      ...
      <div class='addTaskModel' v-show='form.controlMode === 2'>
        ...
        <div class='timeZhouQi'>
          <div @click='clickZhouQi(1)' :class='{active: form.weeks.indexOf(1) > -1}'>星期一</div>
          <div @click='clickZhouQi(2)' :class='{active: form.weeks.indexOf(2) > -1}'>星期二</div>
          <div @click='clickZhouQi(3)' :class='{active: form.weeks.indexOf(3) > -1}'>星期三</div>
          <div @click='clickZhouQi(4)' :class='{active: form.weeks.indexOf(4) > -1}'>星期四</div>
          <div @click='clickZhouQi(5)' :class='{active: form.weeks.indexOf(5) > -1}'>星期五</div>
          <div @click='clickZhouQi(6)' :class='{active: form.weeks.indexOf(6) > -1}'>星期六</div>
          <div @click='clickZhouQi(7)' :class='{active: form.weeks.indexOf(7) > -1}'>星期日</div>
        </div>
      </div>
      <div class='addTaskModel' v-show='form.controlMode === 2'>
  ...
  methods: {
    ...
    save () {
      ...
      if (this.form.controlMode != 1) {
        // 如果不是手动模式
        // 判断时间是否不为空
        if( !this.form.startTime || !this.form.startDate || !this.form.endDate || this.form.weeks.length == 0 ){
          this.$message({
      ...
      routeJobPath({
        ...
        var startDate = this.form.startDate ?
          ...
          +(this.form.startDate.getMonth()+1>9?this.form.startDate.getMonth()+1:"0"+(this.form.startDate.getMonth()+1))+"-"
          +(this.form.startDate.getDate()>9?this.form.startDate.getDate():"0"+this.form.startDate.getDate()) : "";

        var endDate = this.form.endDate ?
          this.form.endDate.getFullYear().toString()+"-"
          +(this.form.endDate.getMonth()+1>9?this.form.endDate.getMonth()+1:"0"+(this.form.endDate.getMonth()+1))+"-"
          +(this.form.endDate.getDate()>9?this.form.endDate.getDate():"0"+this.form.endDate.getDate()) : "";

        var startTime = this.form.startTime ?
            (this.form.startTime.getHours()>9?this.form.startTime.getHours():"0"+this.form.startTime.getHours())+":"
          +(this.form.startTime.getMinutes()>9?this.form.startTime.getMinutes():"0"+this.form.startTime.getMinutes()) : "";
        ...
        createRouteJob({
          ...
          "weeks": this.form.weeks.join(",")
            ...
            // 添加新的任务给当前选中的农机
            this.$parent.$refs.deviceList.addTask(json);
            // 隐藏自己
            this.$parent.showAddTask = false;
            // 显示任务列表
            ...
            // 在列表中选中自己
            this.$parent.$refs.taskList.showInfo(json)

            // 为每一个线段生成路段配置
            this.createPathList(path,0,()=>{
              // 初始化方法z
              this.init();
              this.graphic && this.graphic.layer && this.graphic.layer.remove(this.graphic);
              this.$message({
                type:"success",
                message: '添加成功',
              })
    ...
    draw () {
      ...
      this.mapdraw = map3d.Util.draw("polyline", (res) => {
        ...
        // 显示设备路线
        this.graphic = this.$parent.$refs.deviceList.showTaskPath(item,true);
      })
    },
      ...
      map3d.Util.draw("polygon", (res) => {
        ...
        for (var x = 0; x <= count; x++) {
          ...
          for (var y = 0; y <= count; y++) {
            // 创建新点
            var point = map3d.Util.createPoint([xy1[0] + xdiff * x, xy1[1] + ydiff * y], view.spatialReference);
        ...
        // 将结果绘制成线段
        var geometry = map3d.Util.createPolyline(points, view.spatialReference.wkid);
        var graphic = map3d.Util.createGraphic({
          geometry,
          symbol: {
            type: "line-3d",
