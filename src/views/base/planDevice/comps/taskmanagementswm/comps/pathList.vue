<template>
  <div class="pathLine" v-show="isShow">
    <!-- 路线配置 -->
    <div class="pathList">
      <div class="pathsName">
        <span>路线配置</span>
        <div>
          <!-- <el-button type="primary" size='mini' v-show='skm' @click='backEdit'>完成编辑</el-button>
          <el-button type="primary" size='mini' v-show='false' @click='update'>保存</el-button> -->
        </div>
      </div>
      <div class="pathsModelBox">
        <div
          class="pathsModel"
          v-for="(it, i) in pointTask.filter((e, i) => {
            return activePath < 0 || activePath == i;
          })"
          :key="i"
        ...
        >
          <div class="modelName">
            <span>{{ it.name }}</span>
            <!-- <span :class='{ active: activePath == i }' :style='"color:"+it.color'>*</span> -->
            <input
              style="width:20px;"
              type="color"
              @click.stop=""
...
export default {
  ...
  mounted() {
    ...
    bus.$on("pathlist_show", pointTask => {
      this.isShow = true;
      pointTask && this.setPointTask(pointTask);
    });
    bus.$on("pathlist_hide", () => {
      this.isShow = false;
    });
    bus.$on("pathList_getPath", fn => {
      fn(this.pointTask);
    });
    bus.$on("pathlist_switchMode", () => {
      this.isShow = false; // 如果存在选中的 则取消掉上一次选中
      bus.$emit("map_backEditGeometry"); // 清除选中状态
    });
  },
  methods: {
    /* 开始建立websocket链接 */
    ...
    deviceWsUpdateFunction(result) {
      ...
      this.sensorBtn = data.sensorInfos;
    },
    ...
    taskLineDiff(i) {
      bus.$emit("map_getJob", Job => {
        ...
        bus.$emit("map_refreshShowTaskLineByJob", {
          ...Job,
          pointTask: this.pointTask
        });
      });
    },
    ...
    selectPath(it, i) {
      if (this.activePath === i) {
        // 如果是重复点击则退出
        return;
      } else if (this.activePath > -1) {
        // 如果存在选中的 则取消掉上一次选中
        bus.$emit("map_backEditGeometry");
      }
      // 触发它的几何编辑
      bus.$emit("map_getJob", Job => {
        var tsk = Job.pointTask.find(e => {
          return e.id === it.id;
        });
        bus.$emit("map_editGeometry", tsk.graphic, () => {
          // 退出编辑后
          this.activePath = -1;
        });
        this.activePath = i;
